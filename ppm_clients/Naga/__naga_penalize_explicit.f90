...
!
! This routines performs explicit penalization as a RHS contribution.
!------------------------------------------------------------------------------
MODULE naga_mod_penalize_explicit
IMPLICIT NONE
INTERFACE naga_penalize_explicit
  MODULE PROCEDURE naga_penalize_explicit
END INTERFACE
CONTAINS
SUBROUTINE naga_penalize_explicit(info)
...
! Initialise routine
!----------------------------------------------------------------------------
CALL substart('naga_penalize_explicit',t0,info)
info = 0
!----------------------------------------------------------------------------
! Penalize patches
!----------------------------------------------------------------------------
DO ilevel=1,nlevels
  DO ipatch=1,npatches(ilevel)
    ...
    !-----------------------------------------------------------------------
    ! Do the actual explicit and store in velocity array
        ...
        DO j=1-gstw(2),ptcset(ilevel,ipatch)%snx(2,isubl)+gstw(2)
          DO i=1-gstw(3),ptcset(ilevel,ipatch)%snx(1,isubl)+gstw(3)
            uf(ilevel,ipatch)%fld(1,i,j,k,isub) = &
             ...
             & -uf (ilevel,ipatch)%fld(1,i,j,k,isub)) &
             & * chif(ilevel,ipatch)%fld(i,j,k,isub) * penalizationstrength
...
!----------------------------------------------------------------------------
 9999 CONTINUE
CALL substop('naga_penalize_explicit',t0,info)
RETURN
END SUBROUTINE naga_penalize_explicit
END MODULE naga_mod_penalize_explicit
