...
! <http://orbit.dtu.dk/fedora/objects/orbit:114700/datastreams/file_10852999/content>
!
...
#ifdef __DOTIMING
!-------------------------------------------------------------------------
!  Write cumulative timings
!-------------------------------------------------------------------------
CALL naga_time_write(info)
IF (info .NE. 0) THEN
  CALL naga_say(rank,'Naga','Error writing the timings.')
  GOTO 9999
ENDIF
#endif
